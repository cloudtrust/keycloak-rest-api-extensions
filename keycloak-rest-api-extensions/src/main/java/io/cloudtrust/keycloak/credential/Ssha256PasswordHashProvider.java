...
public class Ssha256PasswordHashProvider implements PasswordHashProvider {
    private final String algorithm = "ssha256";

    @Override
    public boolean policyCheck(PasswordPolicy policy, PasswordCredentialModel credential) {
        return policy != null && credential.getPasswordCredentialData().getAlgorithm().equals(algorithm);
    }
    ...
    public PasswordCredentialModel encodedCredential(String rawPassword, int iterations) {
        ...
        try {
            ...
            byte[] hash = digest.digest(concatBytes(rawPassword.getBytes(), salt));

            toEncode = concatBytes(hash, salt);
            String value = "{SSHA256}" + Base64.encodeBytes(toEncode);

            return PasswordCredentialModel.createFromValues(algorithm, salt, 1, value);
    ...
    public boolean verify(String rawPassword, PasswordCredentialModel credential) {
        ...
        try {
            ...
            byte[] result = concatBytes(hash, salt);
     ...
     * @throws IOException if there is an issue when concatenating bytes
     */
    private byte[] concatBytes(byte[] array1, byte[] array2) throws IOException {
